...
/// </summary>
public class TagDataProvider_UnitTests
{
    /// <summary>
    ...
    /// </summary>
    private static TagDataProvider CreateProvider(Func<HttpRequestMessage, Task<HttpResponseMessage>> httpResponder)
    {
    ...
    public async Task GetExistingTags_ValidResponse_ReturnsTags()
    {
        ...
        Assert.Equal(["critical", "urgent"], tags);
    }
    ...
    [Fact]
    [Trait("Category", "Unit")]
    public static async Task GetExistingTags_TagsFieldMissing_ReturnsEmpty()
    {
        var provider = CreateProvider(req =>
        {
    ...
    public async Task GetExistingTags_NullFields_ReturnsEmpty()
    {
        ...
        await Assert.ThrowsAsync<HttpRequestException>(() => provider.GetExistingTags(350));
    }
    ...
    public async Task PatchTags_ValidRequest_Succeeds()
    {
        ...
        await provider.PatchTags(500, ["enhancement"], true);
    ...
    public async Task PatchTags_HttpFailure_LogsWarning()
    {
        ...
        var ex = await Assert.ThrowsAsync<HttpRequestException>(() =>
            provider.PatchTags(600, ["experimental"], false));
    ...
    {
        string[] largeTagList = [.. Enumerable.Range(1, 100).Select(i => $"tag{i}")];
